    ...
    for i in range(num_reps):
        ...
        if basic_block == "B":
            for k in range(3, -1, -1):
                qml.adjoint(qml.CRX(weights[i+num_reps][2*num_qubits+2*k+1], wires = [k, 5]))
                qml.adjoint(qml.CRX(weights[i+num_reps][2*num_qubits+2*k], wires = [k, 4]))
        if basic_block in ["C", "D"]:
            for j in range(len(wires)-1, -1, -1):
                qml.adjoint(qml.CRX(weights[i+num_reps][3*j+2], wires=pairs[j]))
        if basic_block in ["A", "B"]:
            for j in range(len(wires)-1, -1, -1):
                qml.adjoint(qml.CRX(weights[i+num_reps][2*j+1], wires=pairs[j]))
        if basic_block == "D":
            for j in range(len(wires)-1, -1, -1):
                qml.adjoint(qml.RZ(weights[i+num_reps][3*j+1], wires=[j:j+1]))
            for j in range(len(wires)-1, -1, -1):
                qml.adjoint(qml.RX(weights[i+num_reps][3*j], wires=[j:j+1]))
        if basic_block == "D":
            for j in range(len(wires)-1, -1, -1):
                qml.adjoint(qml.RX(weights[i+num_reps][3*j+1], wires=[j:j+1]))
            for j in range(len(wires)-1, -1, -1):
                qml.adjoint(qml.RY(weights[i+num_reps][3*j], wires=[j:j+1]))
        if basic_block in ["A", "B"]:
            for j in range(len(wires)-1, -1, -1):
                qml.adjoint(qml.RY(weights[i+num_reps][2*j], wires=[j:j+1]))
    return qml.state()
